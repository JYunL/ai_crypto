...
# PnL 계산
pnl_data <- read.csv("./ai-crypto-project-3-live-btc-krw.csv")

pnl_data$PnL <- ifelse(pnl_data$side == "buy", 
                       pnl_data$quantity * (pnl_data$price - lag(pnl_data$price)) - pnl_data$fee, 
                       pnl_data$quantity * (lag(pnl_data$price) - pnl_data$price) - pnl_data$fee)
pnl_data$PnL[is.na(pnl_data$PnL)] <- 0

pnl_data$cum_PnL <- cumsum(pnl_data$PnL)

num_trades <- nrow(pnl_data)
min_pnl <- min(pnl_data$cum_PnL)
max_pnl <- max(pnl_data$cum_PnL)

# 결과 파일에 저장
result <- data.frame(
  total_PnL = sum(pnl_data$PnL),
  min_pnl = min_pnl,
  max_pnl = max_pnl
)

write.table(result, file="./PnL_results.csv", sep=",", col.names=TRUE, row.names=FALSE, quote=FALSE)